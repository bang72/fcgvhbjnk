...
test("V5 is Vercel-first and has one standard build path", async () => {
...
test("V5 keeps private conversation feedback", async () => {
...
test("V5 includes protected admin bootstrap and moderation controls", async () => {
  const pkg = JSON.parse(await readFile(path.join(root, "package.json"), "utf8"));
...
test("V5 platform migration and protected control surfaces exist", async () => {
  const migration = await readFile(path.join(root, "db/migrations/0004_mivo_v5_platform.sql"), "utf8");
  const platform = await readFile(path.join(root, "lib/v5-platform.ts"), "utf8");
  const control = await readFile(path.join(root, "app/api/admin/platform/route.ts"), "utf8");
  const appeal = await readFile(path.join(root, "app/api/v5/appeal/route.ts"), "utf8");
  const matchmaking = await readFile(path.join(root, "lib/matchmaking.ts"), "utf8");
  assert.match(migration, /CREATE TABLE IF NOT EXISTS feature_flags/);
  assert.match(migration, /CREATE TABLE IF NOT EXISTS match_pair_history/);
  assert.match(migration, /CREATE TABLE IF NOT EXISTS moderation_cases/);
  assert.match(platform, /v5MatchScore/);
  assert.match(control, /maintenance/);
  assert.match(control, /rolloutPercent/);
  assert.match(appeal, /verifyPassword/);
  assert.match(matchmaking, /featureEnabled\("matchmaking_v5"/);
});
