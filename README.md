...
This source is the V5 branch of MIVO. V5 keeps the Vercel-first Next.js/PostgreSQL architecture and adds a production control layer instead of introducing a second runtime.

### V5 highlights

- Weighted **Vibe Matching 2.0** using vibe overlap, language, optional region preference, trust/safety signals, waiting time, and pair history.
- **Anti-repeat pair history** and a 30-day avoid window after `Don't rematch soon` feedback.
- **Privacy V5** preferences: language, same-region/anywhere matching, MAX-style incognito flag, quiet hours, notification classes, and default retention period.
- **Trust & Safety V5**: safety score fields, timed restrictions, moderation cases foundation, report evidence, credential-verified public appeals, and admin audit trails.
- **Owner Control Center** at `/admin/v5`: maintenance mode, emergency matchmaking/message/media switches, deterministic feature-rollout percentages, announcements, and basic operational metrics.
- **Public appeal page** at `/appeal` for suspended/permanently banned accounts. Passwords are verified against the existing credential hash and are never stored in the appeal record.
- **Feature flags** seeded by migration `0004_mivo_v5_platform.sql`.
- Health endpoint requires the V5 migration before reporting `ready`.

### Upgrade an existing V2.1 database
...
Then open `/api/health`. `requiredMigration` must show `0004_mivo_v5_platform.sql` and `schemaReady` must be `true`.

### Admin V5

Log in with an `ADMIN` account, then use:

- `/moderation` — reports, users, bans, suspensions, staff, audit.
- `/admin/v5` — platform controls, rollout flags, announcements and operating metrics.

Do not commit real `MIVO_ADMIN_PASSWORD`, database credentials, Stripe secrets, Ably keys, or storage credentials.
