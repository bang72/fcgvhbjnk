  ...
  return <main className="moderation-shell admin-console">
    ...
    <div className="admin-layout">
      <aside className="admin-sidebar">
        <div className="admin-brand"><Shield /><div><strong>MIVO Control</strong><small>V5 Trust & Safety</small></div></div>
        <nav>{tabs.map(([key, label, Icon]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}><Icon />{label}</button>)}</nav>
        {role === "ADMIN" && <Link className="admin-refresh" href="/admin/v5"><Gauge /> V5 Control Center</Link>}<button className="admin-refresh" onClick={() => void load()}><RefreshCw /> Refresh data</button>
      </aside>
