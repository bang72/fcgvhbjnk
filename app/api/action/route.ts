...
import { assertOperational } from "@/lib/v5-platform";
...
const ACTION_ERRORS: Record<string, [string, number]> = {
    ...
    MAINTENANCE_MODE: ["MIVO sedang dalam maintenance mode. Coba lagi setelah layanan dibuka.", 503],
    FEATURE_PAUSED_MATCHMAKING: ["Matchmaking sedang dijeda oleh administrator.", 503],
    FEATURE_PAUSED_MESSAGING: ["Pengiriman pesan sedang dijeda oleh administrator.", 503],
  };
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "message.send") {
      await assertOperational("messaging");
