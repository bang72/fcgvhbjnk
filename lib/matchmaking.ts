...
import { assertOperational, featureEnabled, v5MatchScore } from "@/lib/v5-platform";
...
export async function attemptMatch(viewer: Viewer) {
  ...
  const candidates = await database.prepare(
    `SELECT q.*, i.gender, i.region, p.alias, u.public_id, u.trust_score, u.safety_score,
       COALESCE(pref.language_code, 'id') AS language_code, COALESCE(pref.region_match_mode, 'anywhere') AS region_match_mode,
       COALESCE(pair.match_count, 0) AS prior_matches, COALESCE(pair.skip_count, 0) AS prior_skips,
       q.created_at AS candidate_created_at
     FROM matchmaking_queue q
     JOIN users u ON u.id = q.user_id
     JOIN identities i ON i.user_id = q.user_id
     JOIN profiles p ON p.user_id = q.user_id
     JOIN preferences pref ON pref.user_id = q.user_id
     LEFT JOIN match_pair_history pair ON pair.user_a_id = LEAST(?, q.user_id) AND pair.user_b_id = GREATEST(?, q.user_id)
     ...
     WHERE q.status = 'queued' AND q.user_id != ? AND q.expires_at > ?
       ...
       AND NOT EXISTS (
         SELECT 1 FROM match_pair_history mph
         WHERE mph.user_a_id = LEAST(?, q.user_id) AND mph.user_b_id = GREATEST(?, q.user_id)
           AND mph.avoid_until IS NOT NULL AND mph.avoid_until > ?
       )
       AND NOT EXISTS (
  ...
  ).bind(viewer.id, viewer.id, viewer.id, new Date().toISOString(), cutoff, viewer.id, viewer.id, recentCutoff, viewer.id, viewer.id, viewer.id, viewer.id, new Date().toISOString(), skippedPairCutoff, viewer.id, viewer.id).all<CandidateRow>();

  const ownVibes = safeJsonArray(own.vibes_json);
  const ownV5 = await database.prepare(`SELECT COALESCE(p.language_code, 'id') AS language_code, COALESCE(p.region_match_mode, 'anywhere') AS region_match_mode,
    i.region FROM preferences p JOIN identities i ON i.user_id = p.user_id WHERE p.user_id = ?`).bind(viewer.id).first<{ language_code: string; region_match_mode: string; region: string | null }>();
  const useV5 = await featureEnabled("matchmaking_v5", viewer.id);
  ...
  for (const candidate of candidates.results) {
    ...
    const overlap = overlapScore(ownVibes, safeJsonArray(candidate.vibes_json));
    if (!own.expanded && !candidate.expanded && overlap === 0) continue;
    const waitingSeconds = Math.max(0, (Date.now() - new Date(candidate.candidate_created_at).getTime()) / 1000);
    const score = useV5 ? v5MatchScore({
      vibeOverlap: overlap,
      sameLanguage: (ownV5?.language_code ?? "id") === candidate.language_code || (ownV5?.language_code === "bilingual") || candidate.language_code === "bilingual",
      sameRegion: Boolean(ownV5?.region && candidate.region && ownV5.region === candidate.region),
      regionRequested: ownV5?.region_match_mode === "same_region" || candidate.region_match_mode === "same_region",
      trustScore: candidate.trust_score,
      safetyScore: candidate.safety_score,
      priorMatches: candidate.prior_matches,
      priorSkips: candidate.prior_skips,
      waitingSeconds,
    }) : overlap;
  ...
  for (const { candidate, score } of ranked) {
    ...
    const matchStatements = [
      ...
      database.prepare(`INSERT INTO match_pair_history (user_a_id, user_b_id, match_count, successful_count, skip_count, last_match_at, affinity_score, updated_at)
        VALUES (?, ?, 1, 0, 0, ?, ?, ?)
        ON CONFLICT(user_a_id, user_b_id) DO UPDATE SET match_count = match_pair_history.match_count + 1, last_match_at = excluded.last_match_at, affinity_score = excluded.affinity_score, updated_at = excluded.updated_at`)
        .bind(userAId, userBId, now, Math.round(score), now),
...
export async function startSearch(viewer: Viewer, desiredGender: "random" | "woman" | "man", vibes: string[], expanded = false) {
  ...
  await assertOperational("matchmaking");
  const active = await activeRoomForUser(viewer.id);
  if (active) return { state: "matched" as const, roomPublicId: active.public_id };
  ...
  await getDatabase().batch([
    getDatabase().prepare(
      `INSERT INTO matchmaking_queue (id, user_id, desired_gender, vibes_json, expanded, status, language_code, region_mode, safety_snapshot, created_at, updated_at, expires_at)
       SELECT ?, ?, ?, ?, ?, 'queued', COALESCE(p.language_code, 'id'), COALESCE(p.region_match_mode, 'anywhere'), COALESCE(u.safety_score, 100), ?, ?, ?
       FROM users u JOIN preferences p ON p.user_id = u.id WHERE u.id = ?
       ON CONFLICT(user_id) DO UPDATE SET desired_gender = excluded.desired_gender, vibes_json = excluded.vibes_json,
       expanded = excluded.expanded, status = 'queued', language_code = excluded.language_code, region_mode = excluded.region_mode, safety_snapshot = excluded.safety_snapshot,
       claim_token = NULL, claimed_at = NULL, created_at = excluded.created_at, updated_at = excluded.updated_at, expires_at = excluded.expires_at`,
    ).bind(queueId, viewer.id, desiredGender, JSON.stringify(vibes), expanded ? 1 : 0, now.toISOString(), now.toISOString(), expiresAt, viewer.id),
