...
export async function submitConversationFeedback(viewer: Viewer, roomPublicId: string, rating: "great" | "okay" | "skip") {
  ...
  const now = new Date().toISOString();
  const [pairA, pairB] = orderedPair(viewer.id, room.partner_id);
  const avoidUntil = rating === "skip" ? new Date(Date.now() + 30 * 86_400_000).toISOString() : null;
  const successDelta = rating === "great" ? 1 : 0;
  const skipDelta = rating === "skip" ? 1 : 0;
  const affinityDelta = rating === "great" ? 12 : rating === "okay" ? 3 : -18;
  await getDatabase().batch([
    getDatabase().prepare(
    ...
    getDatabase().prepare(`INSERT INTO match_pair_history (user_a_id, user_b_id, match_count, successful_count, skip_count, last_feedback_at, avoid_until, affinity_score, updated_at)
      VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(user_a_id, user_b_id) DO UPDATE SET
        successful_count = match_pair_history.successful_count + excluded.successful_count,
        skip_count = match_pair_history.skip_count + excluded.skip_count,
        last_feedback_at = excluded.last_feedback_at,
        avoid_until = CASE WHEN excluded.avoid_until IS NULL THEN match_pair_history.avoid_until ELSE excluded.avoid_until END,
        affinity_score = GREATEST(-100, LEAST(100, match_pair_history.affinity_score + excluded.affinity_score)),
        updated_at = excluded.updated_at`).bind(pairA, pairB, successDelta, skipDelta, now, avoidUntil, affinityDelta, now),
    getDatabase().prepare(
...
export async function requestSecondChance(viewer: Viewer, roomPublicId: string) {
  ...
  const reconnect = await getDatabase().prepare("SELECT reconnect_policy, incognito_mode FROM preferences WHERE user_id = ?")
    .bind(room.partner_id).first<{ reconnect_policy: "allow" | "connections" | "nobody"; incognito_mode: number }>();
  if (reconnect?.reconnect_policy === "nobody" || reconnect?.incognito_mode) throw new Error("SECOND_CHANCE_UNAVAILABLE");
